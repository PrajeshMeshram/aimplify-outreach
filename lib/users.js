...
export async function getSenderProfile(googleId) {
  const { data, error } = await supabase
    .from('users')
    .select('company_name, sender_name, value_prop, proof_point, onboarded, name')
    .eq('google_id', googleId)
    .single()
...
export async function saveSenderProfile(googleId, { companyName, senderName, valueProp, proofPoint }) {
  const { error } = await supabase
    .from('users')
    .update({
